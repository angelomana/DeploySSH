    ...
    print(json.dumps(data, indent=4, sort_keys=True))  # Salida en formato JSON
            ...
            if lo:
                ...
                try:
                    ...
                    if error:
                        print(error)
                        er = str(num)+' '+str(tod)+' '+str(host)+' '+str(ip)+' '+str(command)+' '+str(error)
                        self.log(er)
                    
                except paramiko.AuthenticationException:
                    print(str(num)+' '+str(tod)+' '+str(host)+' '+str(ip)+" Error de autenticación, revisa las credenciales")
                    error1 = str(num)+' '+str(tod)+' '+str(host)+' '+str(ip)+' '+str(command)+' '+'Error autenticacion'
                    self.log(error1)
                except paramiko.SSHException as e:
                    print(str(num)+' '+str(tod)+' '+str(host)+' '+str(ip)+f" Error de conexión SSH: {e}")
                    error2 = str(num)+' '+str(tod)+' '+str(host)+' '+str(ip)+' '+str(command)+' '+f"Error conexion ssh {e}"
                    self.log(error2)
                except Exception as e:
                    print(str(num)+' '+str(tod)+' '+str(host)+' '+str(ip)+f" Error: {e}")
                    error3 = str(num)+' '+str(tod)+' '+str(host)+' '+str(ip)+' '+str(command)+' '+f"Error{e}"
                    self.log(error3)
                finally:
                    transport = ssh.get_transport()
                    if transport and transport.is_active():
                        print(str(num)+str(tod)+' '+str(host)+' '+str(ip)+" Cerrando sesion ssh ... \n")
                        sesion = str(num)+' '+str(tod)+' '+str(host)+' '+str(ip)+" Cerrando sesion ssh ... \n"+str(command)
                        self.log(sesion)
                        ssh.close()
